...
int main(int argc, char *argv[])
{
    ...
    ofstream outputfile;
    outputfile.open("saidas/tempos-sequencial"+to_string(lm1)+"X"+to_string(cm2)+"-outputfile.txt",ios_base::app);
    ...
    for (int i = 0; i < lm1; ++i)
    {
        ...
        for (int h = 0; h < cm2; ++h)
        {
            ...
            rlinha.push_back(nv);
            outputfile << "c" << to_string((int) res.size()+1) << to_string((int) rlinha.size()) << " " << to_string(nv) << endl; 
        }   
    ...
    outputfile << chrono::duration_cast<chrono::milliseconds>(end - begin).count() << "(ms)" << endl;
    outputfile.close();
    return 0;
}